...
- `shared/`, `index.server.ts`, and `index.client.tsx` define the validated RPC
  ...
  branch-management details. Action links carry agent references and validated
  HTTPS links to pull requests while the workflow waits for manual merge.
...
range. Processed fingerprints include GraphQL node ID and `updatedAt`, so edits
are audited again while rejected feedback is not repeatedly reviewed.

A clean Draft PR is promoted to Ready and checked again for racing feedback.
The Ready gate halts until Retry. Retry gives merge status priority, returns
the PR to Draft when new feedback exists, halts again when it remains open and
clean, and rejects a closed unmerged PR. After merge, the same PR and final
implementation head are verified. The GitHub-reported merge-result commit
