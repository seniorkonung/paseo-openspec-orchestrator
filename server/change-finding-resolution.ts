...
export function changeFindingResolutionPrompt(input: {
  ...
  const resolutionInstruction = input.publicationAlreadyCompleted
    ? "This session is recovering an interrupted workflow. The selected finding already has a valid committed resolution and a verified entry in the review pull request. Do not invoke the review skill, request approvals, create or amend a commit, push, inspect GitHub, or edit the pull request. Call `complete_review_finding` with `{\"mode\":\"acknowledge-existing\"}`."
    : input.alreadyCommitted
      ? "This session is recovering an interrupted workflow. The selected finding is already absent from a valid committed resolution, but its review pull request entry is not complete. Do not invoke the review skill again, do not request the two approvals again, and do not create or amend a commit. Publish the existing commit if needed, derive concise Russian problem and resolution summaries from the selected finding and committed diff, and call `complete_review_finding` in `publish` mode."
  ...
  const completionInstruction = input.publicationAlreadyCompleted
    ? "Call the orchestrator MCP tool `complete_review_finding` with `{\"mode\":\"acknowledge-existing\"}`. If it reports an error, follow its feedback without invoking `gh` and retry the tool."
    : `Publish the current branch with \`git push --set-upstream origin ${input.branch}\` without force and without pushing tags. Do not run \`gh\`, inspect GitHub, or edit any pull request yourself. After publication, call the orchestrator MCP tool \`complete_review_finding\` without asking a third permission, using \`{\"mode\":\"publish\",\"problem\":\"<краткая проблема>\",\"resolution\":\"<краткий итог>\"}\`. Both summaries must be truthful Russian single-line text no longer than 500 characters. For an accepted risk, describe the acceptance and rationale in \`resolution\`; the tool derives the status from the validated report. The tool owns pull request discovery and editing. If it reports an error, follow its feedback without invoking \`gh\` and retry the tool.`;
...
${completionInstruction} Your task ends after \`complete_review_finding\` succeeds. Do not archive the agent or workspace.`;
}
