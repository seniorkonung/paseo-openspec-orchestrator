...
`F<n>`, объясняет проблему и ждёт одного явного решения об исправлении или
принятии риска. После решения агент без второго подтверждения исправляет
артефакты, проверяет результат, создаёт commit, выполняет push и вызывает MCP.
Существующие задачи сохраняются; необходимая реализация оформляется новыми
незавершёнными задачами. MCP подтверждает исчезновение выбранного
