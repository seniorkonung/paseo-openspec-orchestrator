...
Сейчас список проверяет обязательные профили агентов Paseo, Git-ветку и чистоту
рабочего дерева; после них каркас завершается, чтобы следующие шаги можно было
добавлять по одному.
...
отклоняет неоднозначные совпадения и возвращает полный объект вместе с `id` и
настройками. Не сохраняйте профиль в checkpoint: перечитывайте конфигурацию Paseo
непосредственно перед созданием агента.

Paseo не принимает `profileId` в `agents.create`: будущий шаг должен развернуть
поля выбранного профиля в конфигурацию агента. Модель в профиле необязательна;
если она отсутствует, её нужно получить через provider discovery. Это поведение
описано в [официальной документации Paseo](https://paseo.sh/docs/mcp#agent-profiles).
